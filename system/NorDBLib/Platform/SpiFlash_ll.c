...
void W25Q_Read_Manifacture_Device_ID(void)
{
    ...
    uint8_t send = 0x9F;
    CH341WriteSPI( &send, 1);

    uint8_t dummy_byte = 0xA5;
    CH341StreamSPI( &Temp0, &dummy_byte, 1);
    CH341StreamSPI( &Temp1, &dummy_byte, 1);
    CH341StreamSPI( &Temp2, &dummy_byte, 1);
...
void W25qxx_Write2(uint8_t *send, uint32_t addr, int len)                                       //this func read byte by byte 
{
    uint8_t pbuff;
    for(int i=0; i<len; i++)
    {
        pbuff=send[i];
        W25qxx_WriteByte(pbuff,addr);
        addr++;
    }
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void W25qxx_ReadByte(uint8_t *pBuffer, uint32_t ReadAddr)
{
    ...
    CH341WriteSPI(buff, index);
    CH341StreamSPI(&dummy, pBuffer, 1);
...
void W25qxx_ReadMoreByte(uint8_t *recive, uint32_t len, uint32_t ReadAddr_inBytes)
{
    ...
    CH341WriteSPI(buff, index);
    CH341StreamSPI(dummy, recive, len);
...
void W25qxx_Read2(uint8_t *recive, uint32_t addr, int len)                                      //this func read byte by byte
    {
        uint8_t pbuff;
        for(int i=0; i<len; i++)
        {
            W25qxx_ReadByte(&pbuff,addr);
            recive[i]=pbuff;
            addr++;
        }
    }

void W25qxx_Read3(uint8_t *pBuffer, uint32_t ReadAddr, int len)                                 //this func read from addr with given len
{
    CH341ChipSelect(0, true);
    ...
    CH341WriteSPI(buff, index);
    CH341StreamSPI(dummy, pBuffer, len);
